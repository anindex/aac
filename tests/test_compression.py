"""Tests for compression modules: PositiveCompressor and LipschitzCompressor."""
...
# ---------------------------------------------------------------------------
# LipschitzCompressor sentinel masking (formerly tests/test_lipschitz_sentinel.py)
# ---------------------------------------------------------------------------


class TestLipschitzSentinelMasking:
    """Sentinel distances (1e18) must not corrupt the Lipschitz heuristic.

    Without masking, sentinel inputs propagate through the neural network and
    contaminate every output dimension; the heuristic must fall back to 0 for
    any pair touching a sentinel landmark.
    """
